<?php
class OnlineVisitorsCounter extends CApplicationComponent {

	public $keepDataHours = 12;
	public $activeLimitMinutes = 5;
	...
	public function processRequest() {
		...
		$cookie_key = 'site_online_id';
			...
			if (isset(Yii::app()->request->cookies[$cookie_key])) {
				...
				if (($online = OnlineVisitors::model()->findByPk($site_online_id)) !== null) {
			...
			if (empty($online) && ($online = OnlineVisitors::model()->findByAttributes(array(
					'address' => Yii::app()->request->userHostAddress,
			))) !== null) {
			...
			OnlineVisitors::model()->deleteAllByAttributes(array(), 'refresh_last_time < SUBDATE(NOW(), INTERVAL :days DAY_HOUR)', array(':days' => $this->keepDataHours));
	}
}
